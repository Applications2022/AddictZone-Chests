...
public class EditChestGui extends Gui {
    ...
    public EditChestGui(AddictzoneChests plugin, Player player, Chest chest) {
        ...
        this.setItem(45, ItemBuilder.from(Material.ARROW).name(Component.text("§9Zurück")).asGuiItem(inventoryClickEvent -> new EditGui(player, plugin, new ChestService(plugin).getChests()).open(player)));
    ...
    private void setPageItems(Player player, int page){
        ...
        if(paginatedArrayList.isNextPageAvailable()){
            this.setItem(50, ItemBuilder.from(Material.ARROW).name(Component.text("§9Nächste Seite")).asGuiItem(inventoryClickEvent -> new EditChestGui(plugin, player, chest).open(player, paginatedArrayList.getPageIndex()+1)));
        }

        if(paginatedArrayList.isPreviousPageAvailable()){
            this.setItem(48, ItemBuilder.from(Material.ARROW).name(Component.text("§9Letzte Seite")).asGuiItem(inventoryClickEvent -> new EditChestGui(plugin, player, chest).open(player, paginatedArrayList.getPageIndex()-1)));
        }
